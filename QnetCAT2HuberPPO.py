...
def TEST_PPO(theta_test, testing_size=5000, epsilon_clip=0.2):
    # Initialize policy network
    policy_net = PolicyNet(input_size=1, hidden1=50, hidden2=30, action_space=500).to(device)
    policy_net.eval()  # Assume pre-trained for testing
    
    # Initial theta
    initial_theta = np.zeros(testing_size)
    state = torch.FloatTensor([initial_theta]).to(device)  # Shape: (1, testing_size)
    
    # First item with moderate b
    ...
    for i in range(test_length):
        if i == 0:
            ...
            with torch.no_grad():
                logits = policy_net(state.T)  # Shape: (testing_size, action_space)
                logits[:, item_id.T] = -float('inf')  # Mask used items
                action_dist = dist.Categorical(logits=logits)
                action = action_dist.sample().cpu().numpy()  # Shape: (testing_size,)
            item_id = np.concatenate((item_id, action[np.newaxis, :]))
        ...
        if i == 0:
            b_values = item_bank[action, 1]
            theta_0 = np.where(resp[0] == 1, b_values + 1.5, b_values - 1.5)
        else:
            administered_items = item_id[:, np.arange(testing_size)]
            resp_so_far = np.concatenate(resp_history, axis=0)
            idx_full = np.sum(resp_so_far, axis=0) == i + 1
            idx_zero = np.sum(resp_so_far, axis=0) == 0
            idx_norm = np.bitwise_not(idx_full | idx_zero)
            theta_0[idx_full] = state[-1, idx_full].cpu().numpy() + (item_bank[:, 1].max() - state[-1, idx_full].cpu().numpy()) / 2
            theta_0[idx_zero] = state[-1, idx_zero].cpu().numpy() + (item_bank[:, 1].min() - state[-1, idx_zero].cpu().numpy()) / 2
            if np.any(idx_norm):
                theta_0[idx_norm] = MLE_TEST(item_bank[administered_items[:, idx_norm]], resp_so_far[:, idx_norm])[0]

        state = torch.FloatTensor([theta_0]).to(device)
        theta_history.append(theta_0)

        # Theta SE
        se = np.zeros(testing_size)
        for j in range(testing_size):
            items_so_far = item_bank[item_id[:, j]]
            se[j] = theta_se(theta_0[j], items_so_far)
...
plt.savefig('/content/drive/MyDrive/CAT_agent/training_plot_huberPPO.png')
plt.close()
